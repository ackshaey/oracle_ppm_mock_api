...
def establish_user_server
	...
	@password = 'Welcome1'
	@server = 'http://slc05kvt.us.oracle.com:7011/projectsManagement/rest/v1/pjlFollowedTasks'
end
...
jsonMyTasksList = File.read("pjlFollowedTasks.json")
obj = JSON.parse(jsonMyTasksList)
task_list = obj["items"]
task_list.each do |task|
	id = task["attributes"]["Id"]
	if !File.exist?("pjlFollowedTasks/#{id}.json")
		begin
			content = get_task_details(id)
			File.open("pjlFollowedTasks/#{id}.json", "w") do |file|
				file.write content
				file.close
